 ...
 */
#ifndef CLAUDEOGOTCHI_UART_PORT
#define CLAUDEOGOTCHI_UART_PORT UART_NUM_1
#endif
#ifndef CLAUDEOGOTCHI_UART_TX_PIN
#define CLAUDEOGOTCHI_UART_TX_PIN 17
#endif
#ifndef CLAUDEOGOTCHI_UART_RX_PIN
#define CLAUDEOGOTCHI_UART_RX_PIN 18
#endif
#define CLAUDEOGOTCHI_UART_BAUD 115200
...
void uart_link_send(const uint8_t* buf, size_t len) {
    ...
    if(xSemaphoreTake(s_tx_mutex, pdMS_TO_TICKS(200)) == pdTRUE) {
        uart_write_bytes(CLAUDEOGOTCHI_UART_PORT, (const char*)buf, len);
...
void uart_link_send_link(uint8_t link_state) {
    ...
    size_t n = claudeogotchi_build_link(link_state, buf, sizeof(buf));
    uart_link_send(buf, n);
}

void uart_link_send_heartbeat(void) {
    uint8_t buf[CLAUDEOGOTCHI_MAX_FRAME];
    size_t n = claudeogotchi_build_heartbeat(buf, sizeof(buf));
    uart_link_send(buf, n);
}
...
static void rx_task(void* arg) {
    ...
    for(;;) {
        int n = uart_read_bytes(CLAUDEOGOTCHI_UART_PORT, chunk, sizeof(chunk), pdMS_TO_TICKS(100));
        for(int i = 0; i < n; i++) {
...
void uart_link_init(provision_cb_t on_provision) {
    ...
    uart_config_t cfg = {
        .baud_rate = CLAUDEOGOTCHI_UART_BAUD,
    ...
    ESP_ERROR_CHECK(uart_driver_install(CLAUDEOGOTCHI_UART_PORT, RX_BUF_SIZE, 0, 0, NULL, 0));
    ESP_ERROR_CHECK(uart_param_config(CLAUDEOGOTCHI_UART_PORT, &cfg));
    ESP_ERROR_CHECK(uart_set_pin(
        CLAUDEOGOTCHI_UART_PORT, CLAUDEOGOTCHI_UART_TX_PIN, CLAUDEOGOTCHI_UART_RX_PIN, UART_PIN_NO_CHANGE,
        UART_PIN_NO_CHANGE));

    xTaskCreate(rx_task, "claudeogotchi_uart_rx", 4096, NULL, 10, NULL);
    ESP_LOGI(TAG, "uart up: tx=%d rx=%d @%d", CLAUDEOGOTCHI_UART_TX_PIN, CLAUDEOGOTCHI_UART_RX_PIN, CLAUDEOGOTCHI_UART_BAUD);
}
