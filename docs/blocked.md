...
|------|-------------|----------------------------|
| Hosted relay URL | No account/domain provisioned for a public hosted relay. | Installer defaults to `wss://relay.flipper-sonar.dev` (placeholder); `--relay` overrides it and the self-host path (`make relay` / Docker) is fully working and was used for bring-up. |
